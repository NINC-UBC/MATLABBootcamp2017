...
pathname='/Users/jledue/Downloads/Matlab Tutorial C6/VSD_data/';
...
% if you'd like to make use of size again.
...
for i=1:nTrials
    baseline=norm_st(i,:,:,1:t-stim);
    baseline=mean(baseline,4);
    for j=1:nFrames
        dff(i,:,:,j)=100*(norm_st(i,:,:,j)-baseline)./baseline;
    end
end
...
seedRow=71;
seedCol=91;
linearNDX = sub2ind([128 128],seedRow,seedCol);
dat=reshape(dff,128*128,108);
seed_tc=dat(linearNDX,:);
the_map=corr(seed_tc',dat');

the_map=reshape(the_map,128,128);

figure;imagesc(the_map)
